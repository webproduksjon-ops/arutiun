...
def normalize_generated_nav(html):
    import re
    nav = """<nav class=\"site-nav\" aria-label=\"Основная навигация\" data-nav=\"primary\"><a class=\"nav-link nav-primary\" data-nav-group=\"audience\" href=\"../entrepreneurs.html\">Предпринимателям</a><a class=\"nav-link nav-primary\" data-nav-group=\"audience\" href=\"../individual.html\">Для себя</a><details class=\"nav-more\" data-nav-group=\"secondary\"><summary>Ещё <span aria-hidden=\"true\">⌄</span></summary><div class=\"nav-more-panel\"><div class=\"nav-more-group\"><span>Как это работает</span><a class=\"nav-link\" href=\"../approach.html\">Подход и процесс</a><a class=\"nav-link\" href=\"../format.html\">Формат и стоимость</a><a class=\"nav-link\" href=\"../online.html\">Онлайн-работа</a></div><div class=\"nav-more-group\"><span>Материалы</span><a class=\"nav-link\" href=\"../materialy.html\">Все материалы</a><a class=\"nav-link\" href=\"../video.html\">Видео и идеи</a><a class=\"nav-link\" href=\"../cases.html\">Отзывы и кейсы</a></div><div class=\"nav-more-group\"><span>О специалисте</span><a class=\"nav-link\" href=\"../about.html\">О специалисте</a><a class=\"nav-link\" href=\"../contact.html\">Контакты</a></div></div></details><a class=\"nav-cta\" data-nav-group=\"action\" href=\"../diagnostic.html\">Записаться <span>↗</span></a></nav>"""
    return re.sub(r'<nav class=\"site-nav\".*?</nav>', nav, html, count=1, flags=re.S)
...
def page_html(data, body_html):
    ...
    parent_href = '../' + data['parent']
    related_href = '../materialy/' + data.get('related', '') + '.html' if data.get('related') else '../materialy.html'
...
for path in sorted((root / 'content/pages').glob('*.md')):
    ...
    if not data.get('related'):
        raise ValueError(f"{path}: indexable articles require related front matter")
    (out / f"{data['slug']}.html").write_text(normalize_generated_nav(page_html(data, render_body(body))), encoding='utf-8')
    built += 1
